...
public interface MensajeRepository extends JpaRepository<MensajeEntity, Long> {
    ...
    @Query("SELECT m FROM MensajeEntity m WHERE m.receptor.id = :receptorId ORDER BY m.fechaEnvio DESC")
    List<MensajeEntity> findMensajesRecibidos(@Param("receptorId") Long receptorId);
